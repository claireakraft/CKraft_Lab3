...
struct GapEventHandler : Gap::EventHandler {
  /*
   * Implement the functions here that you think you'll need. These are defined
  ...
  // once the bluetooth is connected, print that is has been connected and turn
  // off advertising
  ...
  void onDisconnectionComplete(const DisconnectionCompleteEvent &event) {
    gap.startAdvertising(LEGACY_ADVERTISING_HANDLE, adv_duration_t::forever(),
                         0);
  }
};
...
void setupAdvertisingData() {
  using namespace ble;
  // set up advertising and if there is an error in set up print that there was
  // an error
  if (BLE_ERROR_NONE !=
    ...
    // if the advertising was set up correctly, then start advertising and set
    // the event handler
    gap.startAdvertising(LEGACY_ADVERTISING_HANDLE, adv_duration_t::forever(),
                         0);
...
void measure_temp() {
  ...
  readData[1] = 0x84; // Turn on our temp sensor, and ensure that we read high
                      // to low on our values.
  ...
  while (1) {
    ...
    if (Hvalue) {
      ...
      // if there is an error in writing to the handle then print that there has
      // been an error
      if (x != BLE_ERROR_NONE) {
...
void on_init_complete(BLE::InitializationCompleteCallbackContext *params) {
  // if initialization successful print or if error print
  ...
  // create characteristic with random UUID and random starting value and to be
  // adle to read and indicate
  int test[1];
  test[0] = 1234;
  ReadOnlyGattCharacteristic<int> tempc(
      uuid, test,
